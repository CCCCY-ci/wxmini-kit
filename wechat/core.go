...
type WxapkgItem struct {
	...
	UnpackRestoredFiles   int
	UnpackRestoreWarnings int
}
...
func (u *WxapkgItem) SetCancelledState() {
	u.UnpackErrorMessage = "unpack cancelled"
	u.UnpackStatus = StatusTypeCancelled
}
...
func NewUnpacker(item *WxapkgItem, options *UnpackOptions) *Unpacker {
	return NewUnpackerWithCancel(item, options, nil)
}

func NewUnpackerWithCancel(item *WxapkgItem, options *UnpackOptions, cancel <-chan struct{}) *Unpacker {
	return &Unpacker{
		item:         item,
		options:      options,
		locker:       sync.Mutex{},
...
var errUnpackCancelled = errors.New("unpack cancelled")

func (u *Unpacker) isCancelled() bool {
...
func (r *cancellationReader) Read(p []byte) (int, error) {
	select {
	case <-r.cancel:
		return 0, errUnpackCancelled
	default:
		return r.reader.Read(p)
	}
}

func readerWithCancellation(reader io.Reader, cancel <-chan struct{}) io.Reader {
	if cancel == nil {
		return reader
	}
	return &cancellationReader{reader: reader, cancel: cancel}
}
...
func (u *Unpacker) UnpackWithStatusCallback(callback func(item *WxapkgItem)) {
	...
	setFailureState := func(err error) {
		if u.isCancelled() {
	...
	if !hasError {
		if u.isCancelled() {
			...
			u.item.UnpackRestoreWarnings = restoreResult.warnings
			if summary := sourceRestoreSummary(restoreResult); summary != "" {
				u.item.UnpackCurrentFile = summary
			}
...
func (u *Unpacker) unpackOne(d *WxapkgFileItemStructure) (err error) {
	...
	if beautify {
		data, err := io.ReadAll(readerWithCancellation(section, u.cancel))
		if err != nil {
	...
	written, copyErr := io.Copy(output, readerWithCancellation(section, u.cancel))
...
func (u *Unpacker) unpack(thread int, callback func(item *WxapkgItem)) bool {
	...
	stopAndReport := func(status StatusType, message string) {
		callbackLocker.Lock()
		...
		hasError = true
		if status == StatusTypeCancelled {
			u.item.SetCancelledState()
		} else {
			u.item.SetErrorState(message)
		}
		snapshot := *u.item
		u.locker.Unlock()
		stop()
	...
	reportError := func(err error) {
		if u.isCancelled() {
			stopAndReport(StatusTypeCancelled, errUnpackCancelled.Error())
			return
		}
		...
		stopAndReport(StatusTypeError, message)
	}
		...
		go func() {
			...
			for {
				...
				case <-done:
					return
				case <-u.cancel:
					reportError(errUnpackCancelled)
					return
				case d, ok := <-chFiles:
					if !ok {
						return
